...
/** Host Cordis 插件名. */
const PLUGIN_NAME = "dsh-notification";
/** 默认通知标题. */
...
const Config = z.object({
	notifyOnIdle: z.boolean().default(true).description("Notify when an agent finishes a turn (status flips to idle).").volatile(),
	notifyOnError: z.boolean().default(true).description("Notify when a step or turn errors (agent/error).").volatile(),
	notifyOnApproval: z.boolean().default(true).description("Notify when a tool call is waiting for user approval.").volatile(),
	minTurnDurationMs: z.number().min(0).default(DEFAULT_MIN_TURN_MS).description("Only notify for turns that ran at least this long, to skip quick replies.").volatile(),
	desktop: z.boolean().default(true).description("Show a native desktop notification on the machine running the dsh server.").volatile(),
	browser: z.boolean().default(true).description("Show browser Notification popups in the Web UI.").volatile(),
	browserOnlyWhenHidden: z.boolean().default(true).description("Only show browser notifications while the tab is hidden.").volatile(),
	webhookUrl: z.string().default("").description("Optional URL to POST a JSON payload to (Slack-compatible text field included).").volatile(),
	title: z.string().default(DEFAULT_TITLE).description("Title used for desktop and browser notifications.").volatile()
});
/**
